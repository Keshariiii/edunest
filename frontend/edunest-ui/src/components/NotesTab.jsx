      ...
      {notes && notes.length > 0 ? (
          <div className="w-full">
            {noteChunks.map((chunk, pageIndex) => (
              <div key={pageIndex} className={`bg-white dark:bg-[#121212] rounded-xl border border-gray-200 dark:border-[#262626] shadow-2xl overflow-hidden min-h-[400px] mb-8 print-force-show print:!mb-8 print:!min-h-0 print:!shadow-none print:break-inside-avoid ${pageIndex === currentNotePage ? `block ${noteDirection === 'next' ? 'animate-slide-next' : 'animate-slide-prev'}` : 'hidden'}`}>
                {/* IDE Window Header */}
                <div className="bg-gray-50 dark:bg-[#171717] border-b border-gray-200 dark:border-[#262626] px-4 py-2.5 flex items-center">
                  ...
                  <div className="mx-auto font-mono text-[10px] text-gray-500 flex items-center gap-2">
                    <span className="text-gray-600 dark:text-gray-400 font-bold">notes.md</span>
            ...
            {noteChunks.length > 1 && (
              <div className="flex gap-2 mt-2 w-full justify-center px-2 font-mono">
                <button onClick={() => { setNoteDirection('prev'); setCurrentNotePage(Math.max(0, currentNotePage - 1)); }} disabled={currentNotePage === 0} className="px-4 py-2 bg-white dark:bg-[#121212] border border-gray-200 dark:border-[#262626] text-gray-600 dark:text-gray-400 rounded-md text-xs hover:text-gray-900 dark:text-white hover:border-gray-300 dark:border-[#404040] disabled:opacity-30 transition-all">&lt; Prev</button>
                <div className="flex items-center justify-center px-4 text-gray-500 text-xs">{currentNotePage + 1} / {noteChunks.length}</div>
                <button onClick={() => { setNoteDirection('next'); setCurrentNotePage(Math.min(noteChunks.length - 1, currentNotePage + 1)); }} disabled={currentNotePage === noteChunks.length - 1} className="px-4 py-2 bg-gray-900 dark:bg-white text-white dark:text-black rounded-md font-bold text-xs hover:bg-gray-200 disabled:opacity-30 transition-all">Next &gt;</button>
              </div>
            )}
          </div>
      ) : (
