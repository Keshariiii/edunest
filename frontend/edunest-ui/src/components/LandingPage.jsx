  ...
  return (
    <div className="w-full">
      ...
      <section className="relative flex flex-col items-center justify-center text-center min-h-[88vh] px-6 overflow-hidden">
        {/* Ambient glow */}
        <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-indigo-600/20 dark:bg-indigo-600/8 rounded-full blur-[130px] pointer-events-none" />
        <div className="absolute bottom-0 right-1/3 w-[350px] h-[350px] bg-purple-600/20 dark:bg-purple-600/8 rounded-full blur-[110px] pointer-events-none" />

        {/* Version badge */}
        <motion.div {...fadeUp(0)} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-gray-50 dark:bg-[#111] border border-[#222] text-[11px] font-mono text-gray-500 mb-8">
        ...
        <motion.div {...fadeUp(0.21)} className="flex flex-col sm:flex-row items-center gap-3">
          <button
            ...
            className="group flex items-center gap-2 bg-gray-900 dark:bg-white text-white dark:text-black px-7 py-3.5 rounded-xl font-bold text-sm hover:bg-gray-100 active:scale-[0.97] transition-all shadow-[0_0_24px_rgba(255,255,255,0.12)] hover:shadow-[0_0_36px_rgba(255,255,255,0.2)]"
          >
            Start Learning Free
          ...
          <button
            ...
            className="flex items-center gap-2 px-7 py-3.5 rounded-xl font-mono text-sm text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-[#262626] bg-gray-50 dark:bg-[#111]/60 hover:border-gray-300 dark:border-[#3a3a3a] hover:text-gray-900 dark:text-white transition-all"
          >
            <UploadCloud size={15} />
            See how it works
          </button>
        ...
        {/* Stats bar */}
        <motion.div {...fadeUp(0.28)} className="mt-14 grid grid-cols-2 sm:grid-cols-4 gap-px bg-gray-100 dark:bg-[#1e1e1e] rounded-2xl overflow-hidden border border-gray-200 dark:border-[#1e1e1e] w-full max-w-xl shadow-md dark:shadow-xl">
          {stats.map(({ value, label, icon: Icon }) => (
            <div key={label} className="bg-gray-50 dark:bg-[#0d0d0d] px-5 py-4 flex flex-col items-center gap-1 hover:bg-gray-50 dark:bg-[#111] transition-colors">
      ...
      <section ref={howRef} className="relative max-w-4xl mx-auto px-6 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {steps.map(({ step, title, desc }, i) => (
            <motion.div key={step} {...fadeUp(i * 0.08)} className="group relative bg-gray-50 dark:bg-[#0d0d0d] border border-gray-200 dark:border-[#1a1a1a] rounded-2xl p-7 hover:border-indigo-500/25 hover:bg-gray-50 dark:bg-[#0f0f0f] transition-all duration-300 hover:-translate-y-1">
      ...
      <section className="max-w-4xl mx-auto px-6 py-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {features.map(({ icon: Icon, title, desc, color, bg, glow, border }, i) => (
            <motion.div key={title} {...fadeUp(i * 0.07)} className={`group flex flex-col h-full bg-[#fafafa] dark:bg-[#0a0a0a] border border-gray-200 dark:border-[#1a1a1a] rounded-2xl p-7 transition-all duration-300 hover:-translate-y-1 hover:bg-gray-50 dark:bg-[#0d0d0d] ${border}`}>
        ...
        <div className="max-w-4xl mx-auto flex flex-col items-center gap-3">
          <div className="flex items-center gap-2.5">
            <div className="w-6 h-6 rounded-md bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center">
              <GraduationCap className="text-gray-900 dark:text-white" size={13} />
            </div>
